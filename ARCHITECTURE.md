...
| `src/diagnostics.rs` | `render` — rustc-style error renderer: snippet + filtered traceback; `lur: <body>` fallback when no source snippet can be rendered. Consumed by `runtime.rs` (one-shot) and `serve.rs` (handler/cron errors). |
