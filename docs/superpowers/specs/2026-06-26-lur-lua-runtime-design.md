...
3. **Each pooled VM independently runs `app.lua` at warm-up** to collect its handler registrations
   (`lur.serve.http(...)`, etc.). Handlers are Lua functions and are **VM-bound**, so "run once and
   share" is impossible — every VM holds its own closures, keyed by a stable handler id; the host
   owns the route table (`method`+`path` → id). Because the top level runs **N times** (once per VM),
   it must be **idempotent** (which composes with §6's `CREATE TABLE IF NOT EXISTS`); genuinely
   once-only setup self-guards (e.g. a `lur.kv` flag) or uses a reserved `lur.serve.on_start` hook.
4. Start each `Source` (HTTP listener, cron scheduler, ...) and enter the event loop.
5. On an incoming event → resolve route → id → borrow a VM → run that id's handler → return the VM.
6. All cross-request / cross-event state lives host-side (see §6); the VM is stateless **per the
   isolation model below**, not by magic.

#### Cross-request isolation in the pool

A pooled VM serves many requests in sequence, so state left behind by one request can bleed into the
next. Probe findings on Luau pin down exactly what is and isn't safe:

- **Locals** (declared inside a handler) are fresh per call → safe.
- **Globals** are **not** safe by default: `sandbox(true)` freezes the *existing* base libraries but
  **does not stop a handler creating a *new* global**, and that global **persists to the next request
  on the same VM** (probe-verified: a value written by one request was readable by the next). lur
  therefore runs **each handler invocation in a fresh per-call environment** — a throwaway table whose
  reads fall through to the readonly globals but whose **writes are discarded after the call** (probe-
  verified to isolate global writes). This closes the global-bleed vector.
- **Upvalues** captured by a handler closure **persist across requests on the same VM** and this is
  **inherent to closures — it cannot be prevented**. So it is a **documented rule**: never stash
  request-scoped data in an upvalue or global; intentional cross-request state goes through
  `lur.state` / `lur.kv` (§6). The mental model is serverless (AWS Lambda): the execution context is
  reused; only per-call locals are private.
...
pool shares it. The Lua state inside each VM is treated as stateless — enforced for globals and
locals, conventional for upvalues, per the §3 *Cross-request isolation* model.
...
- **cross-request isolation**: a handler that writes a global on request A must not leak it to
  request B on the same pooled VM (per-call env discards global writes); each pooled VM registers
  handlers from `app.lua` independently.
